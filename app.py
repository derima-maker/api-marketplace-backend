...
print(f"🔧 ENCRYPTION_KEY: {'✅ SET' if ENCRYPTION_KEY else '❌ NOT SET (will generate temp key)'}")
...
def generate_unique_purchase_key():
    ...
    return 'mp_' + secrets.token_urlsafe(32)  # 'mp' = marketplace key
...
class Product(db.Model):
    ...
    # NEW: Gateway configuration fields
    auth_type = db.Column(db.String(20), default='header')  # 'header', 'bearer', 'query', 'basic'
    auth_header_name = db.Column(db.String(50), default='X-API-Key')  # e.g., 'Authorization', 'X-API-Key'
    creator_api_key = db.Column(db.String(500))  # The creator's master key (ENCRYPTED!)
    rate_limits = db.Column(db.Text)  # JSON: {"Free": "100/day", "Pro": "10000/month"}
...
class Purchase(db.Model):
    ...
    key = db.Column(db.String(100), nullable=False, unique=True)  # Now unique per purchase
...
def register():
    ...
    user = User(email=email, password_hash=hashed.decode('utf-8'), name=name, verified=True)  # Auto-verify for demo
    db.session.add(user)
    db.session.commit()
...
def purchase_product(current_user, product_id):
    ...
    if existing:
        if existing.is_active():
            ...
            # Reactivate expired subscription with new key
...
def api_gateway(product_id, endpoint):
    ...
    if request.args:
        # Remove api_key from forwarded params
    ...
    try:
        ...
        # 12. Return the response to the consumer
...
def test_proxy(current_user):
    """Test endpoint to verify gateway configuration (uses JWT auth, not API key)."""
    data = request.json
    ...
    # Prepare headers with creator's key
